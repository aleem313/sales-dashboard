        ...
        {steps.map((step) => {
          ...
          return (
            <div key={step.label} className="flex items-center gap-3">
              <div className="w-[120px] shrink-0 text-[10px] tracking-[0.05em] text-muted-foreground">
                {step.label}
              </div>
              <div className="flex-1 rounded-[3px] bg-secondary" style={{ height: 20 }}>
                <div
                  className="flex h-full items-center rounded-[3px] pl-2 text-[10px] font-medium text-black transition-all duration-500"
                  style={{
                    width: `${widthPct}%`,
              ...
              <div className="w-10 shrink-0 text-right text-[11px] text-muted-foreground">
                {step.count}
              </div>
            </div>
          );
