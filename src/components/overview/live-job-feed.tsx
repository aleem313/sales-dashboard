  ...
  return (
    <div className="rounded-xl border border-border bg-card shadow-sm">
      <div className="flex items-center justify-between border-b border-border px-5 py-3.5">
        <div>
          <h3 className="text-[13.5px] font-bold">Live Job Feed</h3>
        ...
        <div className="flex items-center gap-3">
          ...
          <Link
            href="/jobs"
            className="rounded-[7px] border border-border px-3 py-1 text-[11.5px] font-semibold text-muted-foreground transition-colors hover:bg-secondary hover:text-foreground"
          >
            View all
          </Link>
        </div>
          ...
          <thead>
            <tr>
              {["Job Title", "Budget", "Profile Match", "Client", "Received"].map((h) => (
                <th
                  key={h}
                  className="border-b border-border px-4 py-2.5 text-left text-[11px] font-semibold uppercase tracking-[0.07em] text-muted-foreground"
                >
                  {h}
                </th>
              ))}
          ...
          <tbody>
            {jobs.map((job) => (
              <tr key={job.id} className="hover:bg-secondary/50">
                <td className="border-b border-border px-4 py-2.5 text-[13px] font-medium">
                  <div className="max-w-[300px] truncate">{job.job_title}</div>
                </td>
                <td className="border-b border-border px-4 py-2.5 font-mono-data text-[12px]">
                  {budgetLabel(job)}
                </td>
                <td className="border-b border-border px-4 py-2.5 text-[12px] text-muted-foreground">
                  {job.profile_name ?? "—"}
                </td>
                <td className="border-b border-border px-4 py-2.5 text-[12px]">
