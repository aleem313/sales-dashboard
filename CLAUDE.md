...
- **Active workflow**: "multiple webhooks" (EWnZg3svZWwcIRs4) — 8 Vollna webhooks per agent (Sana, Laiba, Khansa, Saim, Shayan, Craig, Rebekah, Nawal) → Claude AI proposals → Board task creation → Dashboard webhook. Full PRD: `docs/n8n_workflow_prd.md`.
- **Sub-workflow**: `_relevancy-classifier-core` (hi71jhPU8tmq7hEp) — **inactive**, awaiting Phase 7 splice. Built 2026-05-11. 14 nodes: deterministic gate pre-check (JS) + Gemini Flash 2.5 LLM call (Mode A prompt, ~7,000 tokens) + threshold application + audit-log persist via `/api/relevancy-scores`. Smoke-tested end-to-end (execution 13356: Shayan + synthetic SaaS job → proceed/91/apply_now in 18.2s). Full PRD: `docs/n8n_relevancy_classifier_core_prd.md`. Canonical prompt: `docs/relevancy/mode_a_prompt.md`.
