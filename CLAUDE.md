...
10. **Funnel KPIs and pipeline stages are CUMULATIVE and HISTORY-ACCURATE** (as of 2026-04-27). Every task — not just Lost — counts only toward stages it has actually visited per `activity_log`, not by funnel-order assumption. Implemented via an `activity_history` CTE in `getKPIMetrics` and `getPipelineStages` that aggregates every column name from `task_moved` activity_log entries per task. The per-task `visited` array = history ∪ {current column} (so tasks with no moves still count toward their current column). Each cumulative stage tests array overlap (`&&`) against the funnel columns from that stage onward. Examples: a Won card moved Proposal Submitted → Won counts in Proposals Sent + Won only (NOT Meetings Booked it never visited). A Lost card moved Proposal Submitted → In Chat → Lost counts in Proposals Sent, Proposals Viewed, and In Chat. Won, Lost, Bad Leads (N/A), Untouched (Todo), and On Hold tiles themselves remain current-state. Caveat: relies on `activity_log` being intact — `task_moved` entries are written by `moveTaskAction` (`task-data.ts:1027`); any historical move not logged there is invisible.
