...
10. **Funnel KPIs and pipeline stages are CUMULATIVE** (as of 2026-04-27). A card's count rolls up to every stage it has reached or passed: a card currently in Won counts toward Proposals Sent, Proposals Viewed, In Chat, Meetings Booked, Meetings Done, AND Negotiation. **Lost cards use HISTORY-ACCURATE counting** via `activity_log` — a Lost card only contributes to stages it actually visited (e.g., a card that went Proposal Submitted → Lost counts toward Proposals Sent but NOT Meetings Booked). Implemented via a `lost_visited` CTE in `getKPIMetrics` and `getPipelineStages` that aggregates every column name from `activity_log` task_moved entries per Lost task, then tests with `&&` (array overlap) per stage. On Hold counts toward Proposals Sent only. Won, Lost, Bad Leads (N/A), Untouched (Todo), and On Hold tiles themselves remain current-state.
