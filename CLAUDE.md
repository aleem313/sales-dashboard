...
10. **Funnel KPIs and pipeline stages are CUMULATIVE** (as of 2026-04-27). A card's count rolls up to every stage it has reached or passed: a card currently in Won counts toward Proposals Sent, Proposals Viewed, In Chat, Meetings Booked, Meetings Done, AND Negotiation. Lost cards are assumed to have progressed through the entire funnel before being lost, so they appear in every cumulative bucket (this overcounts cards lost early; the accurate alternative would be activity-log history lookup, deferred). On Hold counts toward Proposals Sent only. Won, Lost, Bad Leads (N/A), Untouched (Todo), and On Hold tiles themselves remain current-state. This means counts only ever go up as cards progress; "Proposal Submitted" is a strict lower bound on activity, not a snapshot of the column.
