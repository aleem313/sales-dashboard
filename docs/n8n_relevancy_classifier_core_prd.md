...
| **Last reviewed** | 2026-05-15 (doc-consistency sweep after primary/failover swap; for live state see §12 changelog) |
| **Workflow ID** | `hi71jhPU8tmq7hEp` |
| **Workflow name** | `_relevancy-classifier-core` |
| **n8n instance** | `ikonicdev.app.n8n.cloud` |
| **Status** | **ACTIVE** (since 2026-05-12 — Phase 7 splice prerequisite). 18 nodes. Primary LLM = DeepSeek R1 via OpenRouter (since 2026-05-15); failover LLM = Gemini 2.5 Flash via OpenRouter. §3–§5 bodies below describe the 1.0 baseline (15-node Gemini-direct topology); §12 changelog records the layered changes that produced the live state. |
...
| **LLM provider (1.0 baseline)** | Google Gemini Flash 2.5 (`models/gemini-2.5-flash`, direct API) | Cred id `0gaoWdarY6itka7l` ("Gemini API (Relevancy Classifier)") — **kept for rollback, not bound to any node since 2026-05-12** |
| **LLM provider (live, primary)** | DeepSeek R1 via OpenRouter (`deepseek/deepseek-r1`) since 2026-05-15 | Cred id `tRUGc5ZmaiQpZEQP` ("OpenRouter (DeepSeek Relevancy Classifier)", openAiApi pointed at OpenRouter base URL) |
| **LLM provider (live, failover)** | Gemini 2.5 Flash via OpenRouter (`google/gemini-2.5-flash`) since 2026-05-12 | Cred id `hEGZwAd3TT4Sthsf` ("OpenRouter (Relevancy Classifier)", openAiApi pointed at OpenRouter base URL) |
...
Key facts:
- **Doc/prompt version:** `v3` (canonical-doc frontmatter; bumped v2→v3 on 2026-06-16 for the agent-feedback tightening — see §12 2026-06-16 (b)). NOTE: the field the LLM *echoes* in its verdict is still `prompt_version: "v1"` (a separate, unchanged emitted value).
...
- **Calibration notes:** 8 non-obvious patterns from observed agent behaviour (8th added 2026-06-16: AI/ML profiles strict-stack-at-gate-1)
...
| Item | Where | Notes |
|---|---|---|
| **DeepSeek-via-OpenRouter credential (PRIMARY)** | n8n credentials id `tRUGc5ZmaiQpZEQP` ("OpenRouter (DeepSeek Relevancy Classifier)") | type: `openAiApi`, base URL `https://openrouter.ai/api/v1`. Bound to the `DeepSeek R1 (OpenRouter)` LLM sub-node since 2026-05-12; promoted to primary 2026-05-15. |
| **Gemini-via-OpenRouter credential (FAILOVER)** | n8n credentials id `hEGZwAd3TT4Sthsf` ("OpenRouter (Relevancy Classifier)") | type: `openAiApi`, base URL `https://openrouter.ai/api/v1`. Bound to the `Gemini 2.5 Flash (OpenRouter)` LLM sub-node since 2026-05-12; demoted to failover 2026-05-15. |
| **Gemini direct credential (LEGACY)** | n8n credentials id `0gaoWdarY6itka7l` ("Gemini API (Relevancy Classifier)") | type: `googlePalmApi`. **Kept in n8n for rollback but no longer bound to any node** since the 2026-05-12 OpenRouter swap. |
...
| **TD-9** | ~~DeepSeek flat-rubric shape drift + empty-output bypassed failover~~ **RESOLVED 2026-06-16** | — | Since ~2026-06-12 DeepSeek R1-Distill (a) returned empty `{output:""}` (a SUCCESS, so `onError` failover never fired → scoreless `review`) and (b) on valid JSON emitted a FLAT `rubric:{client_quality,niche_match,...}` name→number map the 2026-05-15 wrapper normalizer didn't handle → `components`/`total_score` persisted NULL across ALL profiles. Fixed via the `Guard DeepSeek Output` + `Route DeepSeek Output` empty-output→failover pair, DeepSeek `maxTokens` 8192→16384, a flat-rubric→components normalizer in both C6 twins, and a Mode A "NEVER emit a `rubric` field" prompt bullet. See §12 2026-06-16 entry. **Residual:** the normalizer + maxTokens reduce empties but don't eliminate them; the Guard/Route failover is the catch-all. Watch DLQ + `model='gemini-2.5-flash'` rate on the next traffic window — if Gemini-failover share is high, DeepSeek R1-Distill may need a longer maxTokens or a non-reasoning primary. |
...
- **What (4 prompt-text changes, mirrored across BOTH agents + the canonical doc):**
  1. **Gate 8 (location lock-in) — strengthened detection cues.** Added "proof of residency / must provide proof of address or residency"; generalized "<country>-based only" with concrete production examples — "Philippines-based / PH-based only", "must be based in Brazil / Europe", "EU-based required". Kept the existing rule that a client merely being IN country X is not a lock-in unless the freelancer is required to be there. Reason label unchanged ("Location loc"); no new label.
  2. **Gate 4 "Short term job checks" + Gate 7 "Client already conducting an interview" — reinforced soft-signal cues.** Both labels already exist in the 16-enum; no new labels. Tightened detection-cue wording so the LLM more reliably fires them on one-off / micro-task / capped-effort scope (Short term) and active shortlisting / "in talks with candidates" (interviewing), each with an explicit "prefer rejecting" nudge.
  3. **New Calibration Note 8 — conservative per-profile stack calibration.** For AI/ML/automation-centered profile buckets, treat as gate-1 "Out of stack" fails: (a) generic frontend-only work (plain React/Vue/Angular/CSS with no AI/agent/LLM/automation component); (b) no-code platforms (GoHighLevel/GHL, Wix, Bubble, Webflow, FlutterFlow, Squarespace); (c) crypto/web3 builds — UNLESS the job explicitly centers on AI/agent/LLM/automation. Kept conservative (clear cases only; borderline adjacent work like a full-stack app needing some AI glue still passes). Always evaluate against the SPECIFIC profile bucket; never pass on a shared generic language tag alone.
  4. **New OUTPUT RULES bullet — payment-verified as an honest SOFT signal (data-aware, NOT a reject trigger).** When the description / client block does not confirm payment verification, add `"client_payment_verified"` to `missing_signals` AND reduce the `red_flags` component score (more red flags) — but do NOT reject solely on unknown payment status (unknown ≠ unverified; the field is usually absent from input). This surfaces the agents' #1 free-text concern without causing false rejections. **NOTE: true payment-verified GATING requires ingestion of a payment-verified field into the `job` payload — out of prompt scope; tracked as future ingestion work.**
- **Lockstep / asymmetry:** All 4 changes mirrored byte-for-byte across the shared sections of BOTH agent systemMessages — DeepSeek primary (`097c8c70-26ac-4b9c-ad07-2bf296b0125d`, condensed body) AND Gemini failover (`c5-ai-agent`, full body) — in one atomic `n8n_update_partial_workflow` (2 `patchNodeField` ops, 4 patches each). The only intentional divergence remains the evidence-library omission on the DeepSeek body (which carries the full Gemini-body soft-signal cue wording verbatim on the load-bearing cues). Canonical doc `docs/relevancy/mode_a_prompt.md` bumped Prompt version v2→v3 with a changelog row citing the agent-feedback analysis.
- **How:** Single atomic `n8n_update_partial_workflow` (intent logged), 2 `patchNodeField` ops keyed by node ID. Find/replace substrings: gate-8 lock-in line (identical both agents), soft-signal interview+short-term bullets (per-agent wording), calibration-note-7-end → +note 8 anchor, missing_signals bullet → +payment-verified bullet.
- **Verification:** `n8n_validate_workflow profile=runtime` → `valid: true`, 0 errors, 37 warnings (matches the post-2026-06-16 baseline exactly; no new ERROR-level entries; the "AI Agent has no systemMessage" lines are the known validator false-positive — the messages ARE present, confirmed via post-write `mode=full` read showing all 4 changes byte-identical on both agents). versionCounter 98 → 99 (versionId `82976007-0af8-4dc2-89a3-658d61834843`). **Smoke-test deferred to live traffic** (executeWorkflowTrigger isn't externally triggerable). User to verify on the next traffic window: reject rate ticks up modestly (target — closing the "too lenient" gap the agent feedback flagged) and reject reasons skew toward Location loc / Short term / interviewing / Out-of-stack on the newly-caught cases; `missing_signals` now carries `client_payment_verified` on proceeds where payment status is absent.
- **Rollback baseline:** n8n cloud version history id=113 (versionNumber 9, createdAt 2026-06-16 17:03:29, the pre-edit state) — roll back via `n8n_workflow_versions mode=rollback workflowId=hi71jhPU8tmq7hEp versionId=113`. Local anchor file: `backups/workflows bk 16-06-2026/_relevancy-classifier-core (pre agent-feedback prompt-tightening).json`. Inverse: re-apply the 8 patches in reverse (revert gate-8 cues to the 4-phrase list, revert both soft-signal bullets to their pre-edit wording, remove Calibration Note 8, remove the payment-verified OUTPUT RULES bullet) on both agents + revert the doc v3→v2.
- **Scope guardrails honored:** No new hard gates, no new reason labels (16-enum owned by `relevancy-criteria-keeper` + needs a migration). No classifier_mode / threshold / LLM / topology change — systemMessage text + doc only.

### 2026-06-16 — Empty-output failover + flat-rubric normalizer + DeepSeek maxTokens 16384 + prompt tightening (SHIPPED, n8n-only — no commit)

- **Context / regression:** Since ~2026-06-12, classifier scores were persisting `NULL` `total_score` and `components` for ALL profiles (not Craig-specific). Two root causes, verified against live executions 21854 + 21852:
  1. **Empty DeepSeek output bypassed the failover.** The primary DeepSeek agent (R1-Distill) returns `{ output: "" }` (empty/whitespace/non-JSON) on a large fraction of calls when reasoning exhausts the 8192-token budget. An empty string is a SUCCESS, not a thrown error, so the agent's `onError: continueErrorOutput` failover (main[1] → Gemini) NEVER fired. The DeepSeek validator then `JSON.parse("")` → catch → degenerate `{ decision: 'review', total_score: undef, components: undef }`.
  2. **Flat-rubric shape drift.** On the calls where DeepSeek DID emit valid JSON, it emitted a FLAT `rubric: {client_quality:20, niche_match:12, job_freshness:0, budget_attractiveness:0, job_complexity:6, client_communication_quality:9, freelancer_fit_quality:10}` name→number map with non-canonical keys. The 2026-05-15 normalizer only handled the nested `rubric:{components,total_score,tier}` wrapper, so `components` stayed `NULL`.
- **What (6 logical changes, applied over two atomic `n8n_update_partial_workflow` calls — see "How"):**
  1. **`Guard DeepSeek Output` (Code, id `guard-deepseek-output-v3`)** — new node between the primary DeepSeek agent's main[0] and the DeepSeek validator. Probes the agent output: if it's non-empty AND parses to a JSON object with a `decision`, marks `_deepseek_usable: true`; otherwise `false`. Always emits a single merged item carrying `output` (raw DeepSeek text, for the validator), `user_message_json` (echoed from `Prepare Classifier Input`, for the failover agent's `text` expression), and `_deepseek_usable`.
  2. **`Route DeepSeek Output` (IF, id `route-deepseek-output`)** — new node that routes on `_deepseek_usable`: true (main[0]) → `Validate Output + Apply Threshold (DeepSeek)`; false (main[1]) → `AI Agent — Relevancy Classifier (Gemini Failover)`. This is the missing empty-output → failover edge.
  3. **Rewire** primary DeepSeek agent main[0]: was → `Validate Output + Apply Threshold (DeepSeek)`, now → `Guard DeepSeek Output`. (Primary main[1] real-error → Gemini failover unchanged; Gemini main[1] → DLQ unchanged.)
  4. **DeepSeek LLM `maxTokens` 8192 → 16384** (`DeepSeek R1 (OpenRouter)`, id `6b156aeb-...`). R1-Distill spends reasoning tokens before emitting the verdict; 8192 was being exhausted on reasoning, leaving an empty completion. Gemini failover stays at 8192 (it was not the empty-output offender). This reduces — but does not eliminate — empty outputs; the Guard/Route failover is the safety net for the residual.
  5. **Flat-rubric→components normalizer** added to BOTH C6 twins (`54869aed-...` DeepSeek + `c6-validate-threshold` Gemini), inserted BEFORE the existing rubric-wrapper-unwrap block so the flat map is consumed first. Fires only when `components == null` AND `rubric` is an all-numeric flat map. Maps the 7 flat keys onto the canonical 7 components, clamps each to its max, and sums to `total_score` if null: `niche_match→skill_match(30)`, `freelancer_fit_quality→portfolio_evidence(20)`, `client_quality→client_quality(15)`, `job_freshness→competition_position(10)`, `job_complexity→domain_match(10)`, `client_communication_quality→experience_level_fit(10)`, `budget_attractiveness→red_flags(5)`. Byte-identical block in both twins (lockstep preserved; only the `verdict.model` hardcode still diverges).
  6. **Prompt tightening** — a new OUTPUT RULES bullet on BOTH agent system messages (and `docs/relevancy/mode_a_prompt.md`, bumped to Prompt version v2): "NEVER emit a `rubric` field" — forbids the flat name→number map and re-asserts the 7 canonical top-level `components` names with `{value,max,reason}` shape, naming the 6 non-canonical keys to avoid. Prompt is the front-line fix; the C6 normalizer is the safety net.
- **Design deviation from the original spec:** the spec called for a SINGLE Guard Code node with a `main[1]` divert (`return [[item],[]]` / `return [[],[prep]]`). n8n Code nodes are structurally single-output — wiring `main[1]` from a Code node is rejected by the validator ("Output index 1 exceeds output count 1") and would have silently orphaned the failover branch. Split into Guard (Code, computes the flag + carries both downstream shapes) + Route (IF, does the 2-way branch). Behavior is identical to the spec's intent. Sub-workflow node count 18 → 20.
- **How:** Three atomic `n8n_update_partial_workflow` calls. Call 1 (6 logical ops): addNode Guard (initial single-Code version) + rewire + 2× addConnection + updateNode maxTokens + 2× patchNodeField flat-rubric. Validation caught the Code-node multi-output error. Call 2 (correction, 6 ops): removeNode old Guard + addNode Guard(Code) + addNode Route(IF) + 3× addConnection (primary→Guard, Guard→Route, Route[0]→DeepSeek-validate, Route[1]→Gemini-failover). Call 3 (2 ops): 2× patchNodeField prompt tightening on both agents.
- **Verification:** `n8n_validate_workflow profile=runtime` → `valid: true`, 0 errors, 37 warnings (all baseline-noise families; the +4 over the ~33 pre-change baseline are the 2 new nodes' own "$json mode / Invalid $ usage / Code can throw" advisories plus the IF main[1] "missing onError" false positive — main[1] on an IF is the FALSE branch, not an error output). Post-write `mode=full` read confirmed: Guard + Route present and correctly wired; DeepSeek `maxTokens: 16384`; flat-rubric block present in both twins; both agent prompts carry the new bullet. **Smoke-test deferred to live traffic** (executeWorkflowTrigger isn't externally triggerable; the DeepSeek-specific bug only reproduces on a real DeepSeek invocation). User to verify on the next Vollna fire: (a) scores across saim/shayan/craig/sana get non-null `total_score` AND `components`; (b) empty-DeepSeek cases now show a Gemini-produced score (`model='gemini-2.5-flash'`, "via Gemini" badge) instead of a blank `review`; (c) valid-DeepSeek-with-flat-rubric cases get a real numeric score + component breakdown via the normalizer.
- **Rollback baseline:** sub-workflow versions in n8n cloud history (pre-change `versionCounter: 95`; backup file `backups/workflows bk 16-06-2026/_relevancy-classifier-core (pre empty-output-failover fix).json`). Inverse: remove `Guard DeepSeek Output` + `Route DeepSeek Output` and addConnection primary main[0] → `Validate Output + Apply Threshold (DeepSeek)`; revert DeepSeek `maxTokens` to 8192; remove the flat-rubric block from both twins; revert the OUTPUT RULES bullet on both agents + the doc.
- **Resolves:** the flat-rubric drift is logged as TD-9 (resolved same day). Related: the C11 misconfig ([[classifier_c11_misconfig]]) is a SEPARATE open item — this fix reduces (does not eliminate) the LLM-error-as-fake-audit-failure parking by catching empty outputs upstream before they reach the validator/DLQ.

### 2026-05-15 — Primary LLM swap: DeepSeek R1 full → R1-Distill-Llama-70B for latency reduction (SHIPPED, n8n-only — no commit)

- **What:** Swapped the primary LLM model id from the full DeepSeek R1 reasoning model to the DeepSeek R1-Distill-Llama-70B variant. Two coordinated edits in one atomic `n8n_update_partial_workflow` call (2 ops total):
  1. **LLM sub-node model id.** `DeepSeek R1 (OpenRouter)` (id `6b156aeb-60b2-4c0e-afc3-d24f5735f868`) `parameters.model.value`: `"deepseek/deepseek-r1"` → `"deepseek/deepseek-r1-distill-llama-70b"`. All other parameters unchanged: `responsesApiEnabled: false`, `options.maxTokens: 8192`, credential `tRUGc5ZmaiQpZEQP` (OpenRouter DeepSeek key), node type `lmChatOpenAi` v1.3, base URL still OpenRouter `https://openrouter.ai/api/v1`. Node display name `DeepSeek R1 (OpenRouter)` deliberately retained — the model is still in the DeepSeek family, just a faster variant; the dashboard `RelevancyPanel`'s "via X" badge logic (`model.includes("deepseek") ? "DeepSeek R1" : ...`) doesn't need changing.
  2. **DeepSeek-twin Validate Output `verdict.model` hardcode.** `Validate Output + Apply Threshold (DeepSeek)` (id `54869aed-9998-4712-b6a3-513d6005cfe3`) `parameters.jsCode`: the line `verdict.model = 'deepseek-r1';` → `verdict.model = 'deepseek-r1-distill-llama-70b';`. This keeps `relevancy_scores.model` rows distinguishable in the DB — analytics queries can later filter "rows scored by the distilled variant vs by full R1". All other logic in the twin unchanged (rubric-wrapper normalization, gate-9 video verifier, threshold application, ingest-contract field stamping including `job_title`/`job_url`). The Gemini-twin Validate Output (`c6-validate-threshold`, hardcodes `'gemini-2.5-flash'`) is untouched — the lockstep invariant still holds because the model-string hardcode is the one allowed divergence between the twins.
- **Why:** Full DeepSeek R1 was producing per-execution latencies of 3–5 minutes (reasoning-token-heavy), which made the classifier the dominant tail in end-to-end Vollna-to-board latency and noticeably degraded the live "Active mode" UX. The distilled R1-Distill-Llama-70B variant is fine-tuned on the same R1 reasoning traces but eliminates most of the inference-time thinking-token cost — benchmarks suggest 20–40s latency for the same prompt with comparable quality on classification tasks. The prompt body, including the new "top-level shape only" OUTPUT RULES bullet from this morning's normalization fix, applies as-is — no prompt edit needed since the distilled variant inherits R1's reasoning style and output shape from training. Rationale + benchmarks + trade-offs are captured in [`docs/relevancy/llm_options_and_latency.md`](relevancy/llm_options_and_latency.md). User signed off explicitly before this swap was applied.
- **How:** Single atomic `n8n_update_partial_workflow` call with two `patchNodeField` ops referenced by node ID (preferred for surgical edits): (a) `6b156aeb-60b2-4c0e-afc3-d24f5735f868.parameters.model.value` with patches `[{find: "deepseek/deepseek-r1", replace: "deepseek/deepseek-r1-distill-llama-70b"}]`; (b) `54869aed-9998-4712-b6a3-513d6005cfe3.parameters.jsCode` with patches `[{find: "verdict.model             = 'deepseek-r1';", replace: "verdict.model             = 'deepseek-r1-distill-llama-70b';"}]`. No node adds/removes, no connection changes, no credential changes. Sub-workflow node count unchanged at 18, connection count unchanged.
- **Verification:** `n8n_validate_workflow profile=runtime` → `valid: true`, 0 errors, 33 warnings (matches baseline exactly; no NEW errors or warnings introduced). **Smoke-test approach: deferred to live-traffic verification** — `executeWorkflowTrigger` isn't externally triggerable via `n8n_test_workflow`, and synthetic UI execution would not be representative of the production rate-limit / latency profile. User to verify on the next Vollna fire: (a) classifier latency drops from 3–5 min to ~20–40s range; (b) the verdict shape is still correct (real `total_score`, populated `components` with `value` field, all 11 gates including LLM-evaluated ones with concrete evidence strings); (c) the dashboard card renders identically except for the faster availability; (d) `RelevancyPanel`'s "via X" badge still displays "DeepSeek R1" (the dashboard's prefix-match logic accepts the new string); (e) `relevancy_scores.model` rows from this point forward read `'deepseek-r1-distill-llama-70b'` — older rows keep `'deepseek-r1'` (acceptable mixed-string state; analytics queries should account for both).
- **Rollback baseline:** sub-workflow versions in n8n cloud history. Inverse: same two-op atomic mirror — revert `parameters.model.value` to `"deepseek/deepseek-r1"` and `verdict.model` hardcode to `'deepseek-r1';` via `patchNodeField` ops with swapped `find`/`replace`. ~30 seconds to apply if quality regresses.
- **Out of scope (separately tracked):** Working-flow JSON snapshot refresh (TD-4 — flagged as stale post-2026-05-12 + 2026-05-15 changes; deferred). The rationale doc `docs/relevancy/llm_options_and_latency.md` covers other latency-reduction options that were considered and deferred (e.g., dropping to `gemini-2.5-flash` as primary, batching, reducing prompt size).

### 2026-05-15 — C6 validators normalize rubric-wrapper output shape + Mode A OUTPUT RULES clarified to forbid wrapper (SHIPPED, n8n-only — no commit)

- **What:** Two coordinated edits in one atomic `n8n_update_partial_workflow` call (4 ops total):
  1. **Code-side defensive normalization.** Both C6 Validate Output twins (`c6-validate-threshold` Gemini path + `54869aed-9998-4712-b6a3-513d6005cfe3` DeepSeek path) now run a normalization block immediately after `const verdict = { ...llmOutput };` and BEFORE the `if (!ALLOWED.has(verdict.decision))` check. The block (a) unwraps `verdict.rubric → top-level components/total_score/tier` when the LLM nested them inside a `rubric` wrapper, and (b) rewrites `component.score → component.value` when the LLM used the wrong field name. Both rewrites are conditional on the target field being `null`/undefined so the block is idempotent — a Gemini verdict already in canonical shape passes through unchanged. Block applied byte-identically to both twins (matching the lockstep invariant; only the existing `verdict.model` hardcode still differs).
  2. **Prompt-side anti-wrap instruction.** A new bullet inserted into the OUTPUT RULES section of BOTH AI Agent system messages, immediately after the existing "Emit ONLY JSON conforming to the response schema" bullet:
     ...
     Identical bullet on both `097c8c70-...` (DeepSeek primary, condensed ~11.5KB body) and `c5-ai-agent` (Gemini failover, full ~38.7KB body). Canonical doc `docs/relevancy/mode_a_prompt.md` updated to match.
- **Why:** Live execution 16015 (2026-05-15) showed DeepSeek (now primary post-2026-05-15 swap) emitting verdicts in this shape:
  ```json
  {
    "decision": "proceed",
    "gates": { ... },
  ...
  ```
  Two divergences from the schema (defined in the `output-parser` node's `jsonSchemaExample` and assumed by C6): (a) `components` / `total_score` / `tier` were nested inside a `rubric` wrapper instead of being top-level, and (b) inside each component the field was `score` instead of `value`. C6 read `verdict.total_score` (undefined) and `verdict.components` (undefined), so the persisted `relevancy_scores` row had `total_score: null` and `components: null`. Dashboard score badge rendered `—` and the component-breakdown table didn't render at all on every DeepSeek proceed. Gates rendered fine because they ARE at top level in DeepSeek's output. Gemini emits the correct top-level shape per its full-evidence-library prompt — only DeepSeek (with its condensed prompt) drifted into the wrapper shape, and only became user-visible after DeepSeek was promoted to primary on 2026-05-15.
- **How:** Single atomic `n8n_update_partial_workflow` call with 4 `patchNodeField` ops referenced by node ID: (a) `c6-validate-threshold.parameters.jsCode`, (b) `54869aed-9998-4712-b6a3-513d6005cfe3.parameters.jsCode`, (c) `097c8c70-26ac-4b9c-ad07-2bf296b0125d.parameters.options.systemMessage`, (d) `c5-ai-agent.parameters.options.systemMessage`. Both C6 patches use identical `find`/`replace` strings (the line `const verdict = { ...llmOutput };\nif (!ALLOWED.has(verdict.decision)) {`) and insert the same normalization block — preserving the twin-lockstep invariant. Both prompt patches use the same `find` substring (the first OUTPUT RULES bullet) and insert the same new bullet — preserving the primary/failover prompt-equivalence invariant (only the §16 evidence library is allowed to differ between the two prompt bodies).
- **Verification:** `n8n_validate_workflow profile=runtime` → `valid: true`, 0 errors, 33 warnings (matches baseline; no NEW errors or warnings introduced). Targeted post-edit read confirmed: both C6 nodes' jsCode contains the normalization block at the correct insertion point with both rewrites; both AI Agent system messages contain the new OUTPUT RULES bullet at the correct insertion point with identical phrasing. **Smoke-test approach: deferred to live-traffic verification** rather than synthetic — `executeWorkflowTrigger` isn't externally triggerable via `n8n_test_workflow`, and a manual UI execution would not produce a meaningfully different result from the live verification (the rubric-wrapper bug is a DeepSeek behavior, so any DeepSeek invocation is a valid test). User to verify on the next Vollna fire: the dashboard task card for any DeepSeek proceed should now show a real numeric score (not `—`) AND render the component breakdown table. The DB row should have `total_score IS NOT NULL` AND `components IS NOT NULL` on every DeepSeek proceed from this point forward. If DeepSeek (correctly) starts emitting the top-level shape after the prompt edit, the normalization block becomes a no-op safety net — both rewrites are conditional on the canonical fields being missing, so they don't disturb a correctly-shaped verdict.
- **Rollback baseline:** sub-workflow versions in n8n cloud history. Inverse ops: 4 `patchNodeField` ops reversing the same `find`/`replace` (swap the find/replace strings for each op).
- **Out of scope (separately tracked):** Re-enabling the Structured Output Parser on both AI Agents (`hasOutputParser: true`) would catch this class of shape drift at the n8n level. That's PRD TD-2 — separate cleanup, requires upstream n8n LangChain integration bug to be fixed first. Today's fix is belt-and-braces (prompt + code) without changing the parser-inert posture.

### 2026-05-15 — C12 Return Verdict regression fix: extend verdict-recovery chain to include the DeepSeek twin (SHIPPED, n8n-only — no commit)

- **What:** Patched C12 (`c12-return-verdict`) `parameters.jsCode` to fix a regression introduced by the same-day primary/failover swap above. Two changes in one atomic `updateNode` op:
  1. **Recovery-chain extension.** The `safeNode()` fallback that retrieves the verdict when the inbound item is C10/C11's HTTP response (which is `{ok, id}` or `{ok, dlq_id}`, not a verdict) was added a fourth source — the DeepSeek Validate Output twin. New order is primary-first:
     ```js
     let verdict =
       safeNode('Build Reject Payload') ||
       safeNode('Build Review Payload') ||
       safeNode('Validate Output + Apply Threshold (DeepSeek)') ||
       safeNode('Validate Output + Apply Threshold');
     ```
  2. **Synthesized-fallback model field.** When NO upstream verdict can be recovered (both AI Agent paths errored) and C12 synthesizes a `'review'` fallback verdict, `model:` changed from `'gemini-2.5-flash'` (legacy from pre-DeepSeek-failover days, was already wrong since 2026-05-12) to `null`. The dashboard `RelevancyPanel` UI's "via X" badge logic is `model ? ... : null`, so a `null` model now hides the badge entirely on synthesized fallbacks — honest signal that no model actually produced this verdict.
- **Why:** The bug was a silent regression introduced by the 2026-05-15 primary/failover swap. Before the swap, the LLM proceed path went Gemini agent → `Validate Output + Apply Threshold` (Gemini twin) → Decision Switch → Persist Relevancy Score → C12. C12's recovery chain found the verdict in the Gemini twin and stamped it on the response. After the swap, the proceed path goes DeepSeek agent → `Validate Output + Apply Threshold (DeepSeek)` (DeepSeek twin) → Decision Switch → Persist Relevancy Score → C12 — and C12 couldn't find the verdict in any node it knew about (the DeepSeek twin was added 2026-05-12 but C12 was never updated to look at it). Result: every successful DeepSeek `proceed` verdict was being clobbered into the synthesized `'classifier_error_no_verdict'` fallback. Reject/review paths were unaffected because Build Reject/Review Payload runs upstream and C12 finds them fine. User-visible symptom: task cards showed score `—`, tier `marginal`, decision `review`, "via Gemini 2.5 Flash" badge, italic summary "Classifier errored before producing a verdict; routed to review by fallback." — for what were actually successful DeepSeek proceeds. The bug existed since 2026-05-12 in latent form (would have fired only on the rare Gemini-erred-DeepSeek-caught failover path) but became the COMMON case after today's swap made DeepSeek primary.
- **How:** Single atomic `n8n_update_partial_workflow` call with one `updateNode` op on `c12-return-verdict`, referenced by node ID. New `jsCode` body preserves the rest of C12's logic verbatim — `safeNode()` helper definition, `_score_id`/`_dlq_id` stamping from inbound, `classifier_mode` mirroring into `request_meta`. No node adds/removes, no connection changes. Sub-workflow node count unchanged at 18.
- **Verification:** `n8n_validate_workflow profile=runtime` → `valid: true`, 0 errors, 33 warnings (matches baseline). **Live verification follow-up**: next Vollna fire's task card should show a real DeepSeek proceed verdict — non-null score, real tier from the rubric, decision/effective_decision from C6, "via DeepSeek R1" badge, real qualitative summary from the LLM. Synthesized-fallback cards should now hide the model badge entirely (no "via …" pill) and continue to show the "Classifier errored before producing a verdict" italic message. The relevancy_scores DB row also gets `model = 'deepseek-r1'` on success, `model = 'gemini-2.5-flash'` if DeepSeek erred and Gemini caught it, or `model = null` if both errored (the synthesized fallback row, also visible by `summary` containing "Classifier errored before producing a verdict").
- **Rollback baseline:** sub-workflow versions in n8n cloud history. Inverse: single `updateNode` op on `c12-return-verdict` restoring the prior `jsCode` body (drop the DeepSeek-twin `safeNode()` line; restore `model: 'gemini-2.5-flash'` in the synthesized fallback).

### 2026-05-15 — Swap primary/failover LLMs: DeepSeek R1 promoted to primary, Gemini 2.5 Flash demoted to failover (SHIPPED, n8n-only — no commit)

- **What:** Re-wired the failover region so DeepSeek R1 is now the primary LLM and Gemini 2.5 Flash is the failover. Three `rewireConnection` ops (using node IDs to dodge the name-resolution race during the atomic rename) + two `updateNode` rename ops in one atomic `n8n_update_partial_workflow` call. New topology in the failover region:
  ```
  Prepare Classifier Input.main[0] → AI Agent — Relevancy Classifier (DeepSeek-backed, PRIMARY)
    .main[0] success → Validate Output + Apply Threshold (DeepSeek) → Decision Switch
    .main[1] error   → AI Agent — Relevancy Classifier (Gemini Failover)
                         .main[0] success → Validate Output + Apply Threshold → Decision Switch
                         .main[1] error   → Persist to DLQ
  ```
  Node renames (n8n node IDs unchanged — node IDs are stable across renames, only display names changed):
  - `c5-ai-agent` (Gemini-backed agent): `"AI Agent — Relevancy Classifier"` → `"AI Agent — Relevancy Classifier (Gemini Failover)"`
  - `097c8c70-26ac-4b9c-ad07-2bf296b0125d` (DeepSeek-backed agent): `"AI Agent — Relevancy Classifier (DeepSeek)"` → `"AI Agent — Relevancy Classifier"` (now the canonical primary name)
  LLM sub-node bindings are unchanged by the swap — `Gemini 2.5 Flash (OpenRouter)` still feeds the renamed Gemini Failover agent via `ai_languageModel`, and `DeepSeek R1 (OpenRouter)` still feeds the renamed primary agent. The Validate Output twins also stay paired by language model: the Gemini agent still feeds the `Validate Output + Apply Threshold` node (Gemini twin that hardcodes `verdict.model = 'gemini-2.5-flash'`), and the DeepSeek agent still feeds the `Validate Output + Apply Threshold (DeepSeek)` node (hardcodes `verdict.model = 'deepseek-r1'`). Both converge into `Decision Switch` as before. Sub-workflow node count unchanged at **18**; connection count unchanged at **17**.
- **Why:** Operator decision. DeepSeek R1's verdicts have been judged calibration-equivalent (or better on reasoning-heavy borderline cases) to Gemini 2.5 Flash via OpenRouter, with comparable cost. Promoting DeepSeek to primary means Gemini absorbs spillover under DeepSeek's rate-limit pressure instead of the reverse. The DeepSeek system prompt is condensed (~11.5KB, no evidence library) vs. the Gemini one (~38.7KB with evidence library) — that asymmetry was acceptable when Gemini was primary and DeepSeek was a backstop; now it means the primary path has less calibration scaffolding than the failover path. **Outstanding follow-up (out of this commit's scope):** consider promoting the DeepSeek prompt to the full ~38.7KB body to match Gemini's evidence-library fidelity, OR demoting the Gemini prompt to the condensed body. Today's swap preserves the existing two prompt bodies as-is.
- **How:** Single atomic `n8n_update_partial_workflow` call. 3 `rewireConnection` ops + 2 `updateNode` ops, 5 operations total. Connection ops reference both endpoints by node ID (not name) because the rename ops apply within the same atomic batch and the diff validator resolves all node references against the final post-rename state — naming the source/target nodes by old name caused operation 0 to fail with `"To" node not found: "AI Agent — Relevancy Classifier (DeepSeek)"` even though that name existed at the start of the batch. Node IDs are stable across renames, so they work in both pre- and post-rename frames of reference.
- **Verification:** `n8n_validate_workflow profile=runtime` → `valid: true`, 0 errors, 33 warnings (matches pre-swap warning count; all are baseline noise — Code-node lints, `Node is not reachable from any trigger node` false positives on the ai_languageModel sub-nodes, "AI Agent has no systemMessage" false positives on the structure-mode read, "hasOutputParser is not true" warnings on the inert parser per TD-2). Topology confirmed via `n8n_get_workflow mode=structure`: `Prepare Classifier Input.main[0]` now targets `AI Agent — Relevancy Classifier` (DeepSeek-backed) as sole output; primary agent's `main[1]` error output targets `AI Agent — Relevancy Classifier (Gemini Failover)`; failover agent's `main[1]` error output targets `Persist to DLQ`. Live verification: next Vollna fire's `relevancy_scores.model` should be `'deepseek-r1'` on the success path (was `'gemini-2.5-flash'` pre-swap); `'gemini-2.5-flash'` rows now indicate the DeepSeek path errored and Gemini caught the spillover. DLQ rate should drop or stay flat — it cannot rise (the topology absorbs strictly more error scenarios than before, because the prior Gemini-error → DeepSeek-error → DLQ chain is preserved under the new naming, and the only changed condition is which provider gets tried first).
- **Invariant updates required:**
  - `docs/claude/n8n-integration.md` lines 10 + 16: primary/failover labels swapped to reflect DeepSeek-primary, Gemini-failover.
  - `memory/relevancy_classifier_status.md`: 2026-05-12 "DeepSeek R1 failover added" entry now describes the prior topology; current state is recorded in a new 2026-05-15 entry.
  - The `n8n-relevancy-classifier-keeper` agent's system-prompt "critical invariants" block (`ai-agents/n8n-relevancy-classifier-agent.md` if mirrored there, or wherever the live agent definition lives) needs the primary/failover labels swapped — out of this commit's scope; flagged to admin.
- **Rollback baseline:** sub-workflow versions in n8n cloud history. Inverse ops (apply via `n8n_update_partial_workflow` referencing node IDs): rename `097c8c70-26ac-4b9c-ad07-2bf296b0125d` back to `"AI Agent — Relevancy Classifier (DeepSeek)"`, rename `c5-ai-agent` back to `"AI Agent — Relevancy Classifier"`, then rewire `c4-prepare-input.main[0]` from the DeepSeek-id node to `c5-ai-agent`, rewire `c5-ai-agent.main[1]` from `c11-dlq` to the DeepSeek-id node, rewire the DeepSeek-id node's `main[1]` from `c5-ai-agent` to `c11-dlq`. Same 5-op shape, mirror direction.
