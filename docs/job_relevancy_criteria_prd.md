...
|---|---|---|---|---|---|
| 2026-05-12 | v0.2.2 | Always-emit-score policy. The 7-component rubric and `total_score` are now ALWAYS computed and emitted, regardless of decision. Previously the rubric was skipped on `decision=reject` paths (both deterministic-fail and LLM-emit-reject), so `relevancy_scores.total_score` was `null` on rejects. New rule: every verdict carries a numeric score; when `decision=reject`, `tier='reject'` regardless of score (the tier doesn't flip under the operator). Mode A prompt updated on both Gemini + DeepSeek agents in `hi71jhPU8tmq7hEp`; canonical doc `docs/relevancy/mode_a_prompt.md` updated to match. §7 RUBRIC section header changed from "only when decision=proceed or review" to "always required, regardless of decision". No threshold changes; no gate changes; no reason-enum changes. | Calibration / threshold-tuning needs the score distribution across ALL verdicts (reject + review + proceed), not just the proceed/review subset. With null scores on rejects, the dashboard Score Distribution chart (Phase 9) couldn't show "how close to the threshold were the rejects" or "are the proceeds clustered way above the threshold or just barely over" — exactly the questions threshold-tuning asks. Rubric cost is cheap (~200 extra response tokens). | Live classifier emits `total_score` on every row in `relevancy_scores`. Spot-check: `SELECT COUNT(*) FROM relevancy_scores WHERE decision='reject' AND total_score IS NULL AND created_at > '2026-05-12 18:00 ET'` → 0. Tier = `'reject'` for every reject row. | Drafted by Claude in collaboration with Waqas |
