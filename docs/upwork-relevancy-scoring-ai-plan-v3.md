...
**Status legend** (last refreshed 2026-05-13):

- ✅ DONE
- 🟡 PARTIAL — shipped enough to unblock but acceptance criteria not fully met (notes inline)
- ⏳ PENDING — time-gated or ad-hoc, awaiting trigger
- ❌ SKIPPED / NOT STARTED
...
| **5a. ❌ Shared schemas + idempotency middleware** | V1 Zod schemas, I1 idempotency-key middleware, S3 HMAC verification | Dashboard | 1d | All POST endpoints validated + idempotent — *Skipped: inline validation used per codebase convention; idempotency only on `/api/relevancy-scores`; no HMAC anywhere yet. Rolls into Phase 16.* |
| **5b. ✅ Operator Settings API + UI** *(NEW in v3.3)* | `GET /api/admin/system-settings` + 3 PATCH endpoints (§10.6.5) + `<RelevancyClassifierSettings>` card on `/settings` (global toggle, min score, per-profile table, threshold preview, confirmation modals) + cache-bust on save + `activity_log` write | Dashboard | 2d | Admin can flip global Shadow ↔ Active; per-profile toggles enable/disable correctly with global; threshold preview returns live distribution; every change writes to `activity_log` — *activity_log writes deferred: table is task-scoped (FK NOT NULL); current `system_settings.updated_by/updated_at` captures audit. Threshold preview shipped 2026-05-13.* |
| **5c. 🟡 Kill-switch env var + rate-limit middleware** | `RELEVANCY_CLASSIFIER_ENABLED` wired into n8n kill-switch `IF` (§4.2); rate-limit middleware (R1, R2, R4, R6) | Dashboard + n8n-keeper | 4h | Env=`false` reverts to v2 in <30s; rate-limit returns 429 with `Retry-After` — *Kill-switch uses literal "enabled"/"disabled" hack (env vars don't work in n8n cloud expressions); rate-limit only on `/api/relevancy/evaluate-task` so far (R1 done; R2/R4/R6 pending → Phase 16).* |
...
| **7. ✅ Existing workflow splice** *(REVISED in v3.3)* | Insert kill-switch `IF` + `Score Relevancy` executeWorkflow + `Route Verdict` switch (5 branches) + `End (Audit Only)` noOp + the one-line `column: "Todo"` change in `Format ClickUp Task`; follow §13.1 protocol | n8n-keeper | 4h | Mock Vollna job traverses all 5 routing branches based on synthesized verdicts; backup snapshot taken; **`Proposal Submitted` no longer receives auto-created cards** — *K3 routing bug fixed 2026-05-13: switch now reads `classifier_mode_at_decision` + `effective_decision` (top-level) instead of `request_meta.classifier_mode` which Process Job never emitted.* |
| **8. ✅ `job-evaluate-manual` workflow** | NEW: J1-J7 nodes per §4.3; J3 reads `/api/tasks/:id/job-payload`; J5 invokes core | n8n-keeper | 4h | Returns verdict in <3s — *Workflow `fvbhmg0NPnRm4z54` created + activated 2026-05-13.* |
...
| **10. ✅ Admin UI: Relevancy Audit page** | Tiles + drilldowns + L8 lookup + L4 cost dashboard + L11 snapshot-staleness tile + **threshold-flipped rate tile + Settings-history timeline overlay** | Dashboard | 2d | Decision distribution + effective vs raw + gate-fail rates + cost + snapshot freshness + settings-change overlay live — *Multi-reason badges + auto-refresh added 2026-05-13. Job title + URL columns wired via migration 022 same day.* |
| **10a. ❌ Logging + alerts baseline** | L1 pino structured, L2 request_id, L5 Slack alerts, F2/F3/F6 fallback paths | Dashboard | 1d | Trace one job end-to-end through logs — *Not started. Rolls into Phase 16.* |
| **11. ✅ Smoke test** | Replay 20 historical N/A tasks through manual evaluator (against the Shayan/Saim/Craig snapshots) | Waqas | 4h | ≥85% agreement — *18/20 (90%) PASS on 2026-05-13. Caveat: all 20 fixtures hit gate-2 deterministic short-circuit (jobs were 9d old at run time), so LLM rubric + threshold flipping weren't exercised. Plumbing validated end-to-end. See `docs/phase-11-results/README.md`.* |
| **12. ✅ Shadow rollout** *(REVISED in v3.3)* | `system_settings.relevancy.classifier_mode='shadow'` (the migration default). Routing already wired — cards traverse branch 4/5 of the matrix (Todo + badge). Daily decision pivot; admin tunes `min_score` via Settings UI based on live preview. | n8n-keeper + Waqas | 1 week | 7 days × loaded-profile of `relevancy_scores` rows; classifier-vs-agent agreement ≥85%; min_score calibrated — *Ran 2026-05-12 → 2026-05-13. Operator flipped Active after ~1 day rather than the full 7d; data thin but no incidents.* |
| **13. 🟡 Calibration review** | Audit shadow data; set per-profile `min_score_override` for profiles that need it. All 7 active profiles already have snapshots loaded as of 2026-05-11 (Laiba is intentionally unloaded — inactive profile). | Waqas | 2d | Per-profile thresholds documented in PRD changelog; no snapshot uploads pending — *Formal review skipped; operator went Shadow → Active directly. Threshold preview is live in Settings UI for ad-hoc tuning; per-profile `min_score_override` remains null for all profiles (inherits global 50). Re-do if override rate climbs in audit.* |
| **14. ✅ Active rollout** *(REVISED in v3.3)* | Admin flips `/settings` → Relevancy Classifier → Global Mode → Active (with confirm modal). No n8n edit, no redeploy. Cards matching branches 1+2 stop being created within ~60s. | Waqas | 5 min | First "AI-rejected" entry in `relevancy_scores` with `classifier_mode_at_decision='active'`; audit page shows decision distribution updating live — *Flipped 2026-05-13 17:37 UTC by muhammad.ali@ikonicsolution.com. Verified via `scripts/smoke-test-phase-14.ts --mode watch`. K3 routing bug also caught + fixed same day (2 leaked cards moved to N/A with audit trail).* |
| **14a. ⏳ Per-profile staggered activation** *(NEW in v3.3)* | If any profile needs more shadow time, set its `classifier_enabled=false` from Settings UI before flipping global. Re-enable individually as each profile's data converges. | Waqas | ad-hoc | All profiles eventually have `classifier_enabled=true` — *No profile staggered as of 2026-05-13; only triggered if override rate elevated on a specific profile.* |
...
| **16. ❌ P1 hardening pass** | All §16 P1 items (R1/R2/R4/R6, A3-A9/A11, S4/S7/S9-S11, V7/V9, etc.) | Dashboard + n8n | 1 week | All P1 items closed in audit — *Not started 2026-05-13. Absorbs the leftovers from 5a/5c/10a.* |
| **17. ⏳ Post-launch review** | 30-day review; promote stable JSON snapshot to backup; archive previous; document calibrated thresholds + final per-profile settings in PRD changelog | Waqas | 1h | New `(working).json` baseline; `CLAUDE.md` updated — *Earliest 2026-06-12 (30d post-Active).* |
