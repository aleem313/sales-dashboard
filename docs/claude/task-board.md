...
| GET | `/api/events/tasks` | Agent (SSE stream) |
...
- **Real-time new-task notifications (SSE, 2026-06-16)**: the agent board's new-task bell is pushed via Server-Sent Events, not just polling. Flow: `/api/v1/webhooks/tasks` → `emitTaskCreated()` (`src/lib/task-events.ts`, an in-process `EventEmitter` bus — single Contabo process, no Redis) → `/api/events/tasks` SSE route streams to the assigned agent → `<BoardAutoRefresh realtime>` (`board-auto-refresh.tsx`) opens an `EventSource` and fires the bell via `notifyNewTask()` (`src/lib/notified-tasks.ts`). **Gotcha that forced this design:** `useNewTaskNotifier`'s `flatTasks` is bound to the server **`buckets` prop** in `board-view.tsx`, NOT the zustand store — so `refreshBoard` (store-only) never reaches the prop-bound notifier, and the prop only changes on a server re-render. So the SSE handler fires the bell DIRECTLY (not via the board list), and `notifyNewTask` dedups by id so the SSE and polling paths never double-beep. Polling (`runInBackground`) stays as the fallback. Self-hosted, no third-party push service. Full detail: `task_notifications_sse` memory.
