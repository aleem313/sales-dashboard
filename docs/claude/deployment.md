...
- `docker-compose.prod.yml` — full nginx+certbot stack, intended for post-domain setup. Blocked only on a domain → 157.173.110.62 (A record); then switch the deploy from `server.yml` to `prod.yml` + bootstrap the Let's Encrypt cert (certbot webroot). **HTTPS is also a hard requirement for desktop/OS notifications** — the new-task bell's `Notification` API is unavailable on the current plain-HTTP origin (browsers gate it to secure contexts), so agents only get SSE + in-page toast/beep until HTTPS is live. `docker/nginx/nginx.conf` has a dedicated `/api/events/` location with `proxy_buffering off` so the SSE stream isn't stalled by nginx buffering — keep it when editing the proxy.
