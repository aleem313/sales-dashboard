...
> **The old IP `http://157.173.110.62` now 301-redirects** and drops the body on cross-host POSTs, so every external caller was repointed at the domain the same day: all 4 n8n workflows (parent, classifier, proposal-regenerate, job-evaluate-manual) + the `relevancy-dlq-drain.yml` GitHub cron — see `docs/claude/n8n-integration.md` for the node roster. The bare IP is now only for SSH/deploy (`CONTABO_SSH_HOST`). Legacy n8n workflow `ZfwmIaDv8yZOisPx` (Upwork Outbound Machine v3) was left on its dead Vercel+ClickUp endpoints — recommend archiving.
