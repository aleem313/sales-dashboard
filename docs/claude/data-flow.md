...
- **Drill-down:** `getResponseTimeJobs` → `/api/dashboard/response-time-jobs` → `ResponseTimeDrillDown` (a SEPARATE path from the count cards' `getKPIMetricTasks`/`KPIMetricDrillDown`, because the median reads `jobs` not the task CTE). It returns the EXACT same job set (same WHERE + same elapsed expr) sorted by elapsed ASC, each row showing per-job wait time, plus a median marker line — so the headline is verifiable by eye. The modal's median is sourced from `getAvgResponseTime` itself (not recomputed from the ≤500-row list) so it can never drift from the card. ⚠ Keep the two functions' predicates in lockstep. (Drill-down query selects `j.job_title AS title` — the `jobs` column is `job_title`, not `title`; selecting `j.title` 500s.)
- **Agents page uses the SAME definition** (aligned 2026-06-10). The list-page top "Avg Time to Apply" card calls `getAvgResponseTime` directly. Per-agent "Time to apply (avg)" (in `getEnhancedAgentStats`) and the detail page's "Avg Response" (in `getAgentStats`) both overlay `getResponseTimeMedianHoursByAgent` (the same predicate, `GROUP BY agent_id`), replacing the old sent-only `AVG(...)`. This also feeds `getEnhancedAgentStats`'s `speedScore`, so a job left unanswered now counts against that agent's response time. ⚠ `getResponseTimeMedianHoursByAgent` must stay in lockstep with `getAvgResponseTime` too.

## Top Profiles count now includes manual cards + System/Manual drill-down

As of 2026-06-10. The dashboard "Top Performing Profiles" section was undercounting vs the "Agent Leaderboard": the Leaderboard links tasks via `task_assignees` (a LEFT JOIN to jobs) so manual cards count, but `getEnhancedProfileStats` did an INNER `JOIN jobs` and read `j.profile_id`, dropping every manually-created card (no jobs row).

- **Fix:** `getEnhancedProfileStats`'s `profile_scoped_tasks` now attributes a card to a profile by the SAME dual-resolution as `getKPIMetricTasks` / `getConnectsUsageByProfile`: `JOIN profiles pa ON (the card's job → pa.profile_id) OR (a task tag whose name = pa.profile_name)`, with `LEFT JOIN jobs` for won_value/agent_id/stage_entered_at. A card matching two profiles counts under both (mirrors the popup); cards matching neither are excluded. Agent filter extended to also match `task_assignees` (manual cards have no `j.agent_id`). This flows through ALL columns (Proposals, Interview %, Won, win-rate denom) per stakeholder decision. Manual wins have no `won_value`, so revenue stays system-only.
