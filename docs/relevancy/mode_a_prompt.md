...
| Date | Change | Why |
|---|---|---|
| 2026-06-16 (v3) | CONSERVATIVE tightening of 4 description-readable signals, driven by a DB analysis of 98 `agent_feedback` rows (92/98 showed the classifier running too LENIENT — agents catching missed rejections). (1) **Gate 8** location-lock-in cues expanded: added "proof of residency / proof of address or residency", generalized "<country>-based only" with concrete production examples (Philippines/PH-based only, must be based in Brazil / Europe, EU-based required) — reason label unchanged ("Location loc"). (2) **Gate 4 "Short term job checks" + Gate 7 "Client already conducting an interview"** soft-signal detection-cue wording tightened so the LLM fires them more reliably on one-off / micro-task scope and active shortlisting / "in talks with candidates" — no new labels (both already in the 16-enum). (3) New **Calibration Note 8**: for AI/ML/automation-centered profile buckets, treat generic frontend-only / no-code (GHL, Wix, Bubble, Webflow, FlutterFlow, Squarespace) / crypto-web3 as gate-1 "Out of stack" unless the job explicitly centers on AI/agent/LLM/automation; always evaluate against the SPECIFIC profile bucket, never pass on a shared generic language tag alone. (4) New OUTPUT RULES bullet: **payment-verified as an honest SOFT signal** — when payment-verified is unconfirmed, add `client_payment_verified` to `missing_signals` and reduce the `red_flags` component score, but NEVER reject solely on unknown payment status. | Agent-feedback analysis (98 rows, top reasons: Too many invites 29, Out of stack 27, Low rate 16, Red flag/payment-unverified 11, Location loc 9, Short term 5, Client interviewing 4). Dominant SOURCE-DATA pains (payment_verified, proposals_count, client metadata nulls) are ingestion gaps and explicitly OUT OF SCOPE here — this change only tightens what the LLM can read from `job.description` / skills / profile bucket. NO new hard gates, NO new reason labels (16-enum owned by relevancy-criteria-keeper + needs a migration). True payment-verified gating requires ingestion of a payment-verified field (out of prompt scope). Mirrored byte-for-byte across BOTH agent systemMessages (Gemini full + DeepSeek condensed) on the shared sections. |
| 2026-06-16 | OUTPUT RULES: added "NEVER emit a `rubric` field" bullet forbidding the flat name→number rubric map and re-asserting the 7 canonical top-level `components` names with `{value,max,reason}` shape | DeepSeek (R1-Distill primary) regressed to a NEW drift: a FLAT `rubric: {client_quality:20, niche_match:12, job_freshness:0, ...}` name→number map with non-canonical keys (`niche_match`, `freelancer_fit_quality`, `job_freshness`, `job_complexity`, `client_communication_quality`, `budget_attractiveness`). The 2026-05-15 wrapper normalizer only handled nested `rubric:{components,...}` so `components` stayed NULL. Belt-and-braces with a C6 flat-rubric→components normalizer shipped same day (maps the 7 flat keys onto the canonical 7, clamps to max, sums to total_score) — prompt is the front-line fix, code is the safety net. Part of the empty-output-failover fix batch. |
...
| 8 | `8_no_location_lockin` | Job does NOT require the freelancer to be physically located in a specific country/region (US-only, Canada-only, etc.). Lock-in triggers: "U.S. residents only", "must be located in [country]", "American-based", "EU citizens only", "EU-based required", "<country>-based only" (e.g. "Philippines-based / PH-based only", "must be based in Brazil", "must be based in Europe"), "must provide proof of address or residency", "proof of residency required". A client in country X is NOT a lock-in unless the posting demands the freelancer also be in X. | "Location loc" |
...
- **"Client already conducting an interview"** — fire this whenever the description shows the client is ALREADY shortlisting or in active conversations, even if not yet filled. Detection cues: "currently interviewing", "in talks with candidates / freelancers", "shortlisting now", "have a few candidates already", "reviewing applicants this week", "will be interviewing shortly". Distinct from "Already hired" (which means filled). When you see these, prefer rejecting over proceeding. Treat as a gate-7 (`7_job_availability`) failure context.
- **"Short term job checks"** — fire this whenever the description shows clearly one-off / micro-task / capped-effort scope, even if the budget alone looks fine. Detection cues: "one-off", "single task", "quick fix", "small fix", "tiny task", "just need X done", "1-2 hour task", "5 min job", "30 minute task"; or duration "Less than 1 week" / "less than a month" combined with a fixed budget below $200; or scope that obviously caps total effort below ~2 hours (a single bug, one small change, one page). When the scope reads as a micro-gig, prefer rejecting. Treat as a gate-4 (`4_hourly_floor`) failure context.
...
8. **AI/ML/automation profiles: be strict on stack at gate 1.** When the profile's stack bucket is centered on AI agents, LLMs, ML, or workflow automation (the `profile.skills` / `stack_bucket` lead with terms like AI Agent Development, LLM, Machine Learning, RAG, n8n, Workflow Automation), treat the following as gate-1 **"Out of stack"** fails UNLESS the job explicitly centers on AI / agents / LLMs / automation: (a) generic frontend-only work — plain React / Vue / Angular / CSS with NO AI, agent, LLM, or automation component; (b) no-code platforms — GoHighLevel / GHL, Wix, Bubble, Webflow, FlutterFlow, Squarespace; (c) crypto / web3 builds. Keep this conservative — only the clear cases above; do NOT extend it to borderline adjacent work (e.g. a full-stack app that happens to need some AI glue still passes). Always evaluate against the SPECIFIC profile bucket in the input; never pass a job on a shared generic language tag alone (e.g. "JavaScript" appearing in both is not a match).
...
- **NEVER emit a `rubric` field.** Do NOT emit a flat name→number map like `rubric: {client_quality: 20, niche_match: 12, ...}`. The rubric lives ONLY under top-level `components`, keyed by these 7 EXACT names — `skill_match` (max 30), `portfolio_evidence` (max 20), `client_quality` (max 15), `competition_position` (max 10), `domain_match` (max 10), `experience_level_fit` (max 10), `red_flags` (max 5) — and each value is an object `{ "value": <int>, "max": <int>, "reason": "<evidence>" }`. Do NOT invent component names (no `niche_match`, `freelancer_fit_quality`, `job_freshness`, `job_complexity`, `client_communication_quality`, `budget_attractiveness`).
...
- **Payment-verified is a SOFT signal, never a reject trigger.** The input usually does NOT carry an explicit payment-verified field. When the description / client block does not confirm the client's payment method is verified, add `"client_payment_verified"` to `missing_signals` AND reduce the `red_flags` component score (i.e. surface MORE red flags — a lower `red_flags` value). Do NOT reject solely because payment-verified status is unknown — unknown ≠ unverified, and this data is usually simply absent from the input. This surfaces the concern for human review without causing false rejections.
